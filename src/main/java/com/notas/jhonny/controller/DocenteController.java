...
public class DocenteController {
	...
	public  ResponseEntity<?>loginDocente(@PathVariable("user") String user,@PathVariable("pass") String pass) {

		Docente docente = null;
		Map<String, Object>response = new HashMap<>();
		try {
			docente = docenteService.loginDocente(user, pass);
		...
		if(docente == null) {
			response.put("Mensaje", "El docente con Correo: ".concat(user.concat("No existe en la base de datos")));
			return new ResponseEntity<Map<String,Object>>(response,HttpStatus.NOT_FOUND);
		}
